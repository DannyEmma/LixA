...
function renderActiveShape({ cx, cy, midAngle, innerRadius, outerRadius, startAngle, endAngle, payload }: PieSectorDataItem) {
  ...
  const { id, name, density, fill } = payload

  return (
    <g>
      <foreignObject className={styles["middle-text-container"]} x={cx} y={cy}>
        <p className={styles["middle-text"]}>{Number(id) === 1 && name}</p>
      </foreignObject>
...
export default function CustomPieChart({
  ...
  variant: "letter" | "word"
  densities: LetterDensity[] | WordDensity[]
  colors: string[]
}) {
